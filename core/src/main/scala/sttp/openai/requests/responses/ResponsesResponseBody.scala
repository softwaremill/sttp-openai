...
/** @param background
  ...
  *   When the service_tier parameter is set, the response body will include the service_tier value based on the processing mode actually
  *   used to serve the request. This response value may be different from the value set in the parameter.
  * @param status
  ...
  *   * Function calls (custom tools): Functions that are defined by you, enabling the model to call your own code with strongly typed arguments and outputs.
  * @param topLogprobs
  ...
  *     fit the context window by dropping input items in the middle of the conversation. 
...
object ResponsesResponseBody {
  ...
  /** @param effort
    *   Constrains effort on reasoning for reasoning models. Currently supported values are low, medium, and high. Reducing reasoning
