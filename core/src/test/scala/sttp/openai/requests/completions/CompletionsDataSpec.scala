...
class CompletionsDataSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "Given request of single prompt as a JSON" should "be properly serialize to case class" in {
    import sttp.openai.requests.completions.CompletionsRequestBody._
    import sttp.openai.requests.completions.CompletionsRequestBody.CompletionsBody._

    // given
    val request = CompletionsRequestBody.CompletionsBody(
      model = "text-davinci-003",
      prompt = Some(SinglePrompt("Say this is a test")),
      maxTokens = Some(7),
      temperature = Some(0),
    ...
    // when
    val givenRequest: CompletionsBody = SnakePickle.read[CompletionsBody](fixtures.CompletionsFixture.jsonSinglePromptRequest)

    // then
    givenRequest shouldBe request
  }

  "Given request of multiple prompt as a JSON" should "be properly serialize to case class" in {
    import sttp.openai.requests.completions.CompletionsRequestBody._
    import sttp.openai.requests.completions.CompletionsRequestBody.CompletionsBody._
    ...
    // when
    val givenRequest: CompletionsBody = SnakePickle.read[CompletionsBody](fixtures.CompletionsFixture.jsonMultiplePromptRequest)
